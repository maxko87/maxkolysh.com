...
// Preserve the horizontal overview framing on a narrow portrait canvas.
export const overviewFov=(aspect:number)=>T.MathUtils.radToDeg(2*Math.atan(Math.tan(T.MathUtils.degToRad(42)/2)/Math.min(1,Math.max(.3,aspect))));
export function createHouseViewer(container:HTMLDivElement, photoMarkers?: {roomIds:string[];onSelect:(roomId:string)=>void}){
 ...
 const renderer=new T.WebGLRenderer({antialias:true});renderer.setPixelRatio(Math.min(devicePixelRatio,touchDevice?1.5:2));renderer.shadowMap.enabled=true;renderer.shadowMap.type=T.PCFSoftShadowMap;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=1.2;container.appendChild(renderer.domElement);
 ...
 function overview(){walking=false;pressed.clear();orbit.enabled=inputEnabled;viewWalls(false);camera.fov=overviewFov(camera.aspect);camera.updateProjectionMatrix();camera.position.set((active?38:42)*F,(active?65:82)*F,(active?91:98)*F);orbit.target.set(11*F,0,(active?30:21)*F);orbit.update();}
 ...
 renderer.domElement.addEventListener('pointerdown',down);renderer.domElement.addEventListener('pointermove',pointer);renderer.domElement.addEventListener('pointerup',up);renderer.domElement.addEventListener('pointercancel',up);renderer.domElement.addEventListener('lostpointercapture',up);window.addEventListener('keydown',keydown);window.addEventListener('keyup',keyup);window.addEventListener('blur',clear);document.addEventListener('visibilitychange',clear);
 const resize=new ResizeObserver(()=>{const w=Math.max(1,container.clientWidth),h=Math.max(1,container.clientHeight);renderer.setSize(w,h);camera.aspect=w/h;if(!walking)camera.fov=overviewFov(camera.aspect);camera.updateProjectionMatrix();});resize.observe(container);setLevel(0);let previous=performance.now();
 ...
 return {setLevel,overview,visit,move,download,setFurniture,matchPhoto,setInputEnabled,dispose(){renderer.setAnimationLoop(null);markers.forEach(m=>m.button.remove());resize.disconnect();orbit.dispose();window.removeEventListener('keydown',keydown);window.removeEventListener('keyup',keyup);window.removeEventListener('blur',clear);document.removeEventListener('visibilitychange',clear);scene.traverse(o=>{if(o instanceof T.Mesh||o instanceof T.Line){o.geometry.dispose();(Array.isArray(o.material)?o.material:[o.material]).forEach(m=>m.dispose());}});renderer.dispose();renderer.domElement.remove();}};
}
